## 将 L1 Flow 集成到 π0.5 的方案

整体思路：π0.5 的 action expert 本质上是一个 flow matching 模块，L1 Flow 不改变网络结构，只改变**预测目标**（velocity → sample）、**损失函数**（MSE → L1）和**推理调度**（多步 ODE → 2步）。因此可以直接加载原始权重后做轻量 fine-tune。
